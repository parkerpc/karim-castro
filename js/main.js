...
const io = new IntersectionObserver(entries => {
  entries.forEach(e => {
    ...
    if (n && !done.has(n)) {
      ...
      const t = setInterval(() => {
        s = Math.min(s + tg/120, tg);
        if (ac) n.childNodes[0].textContent = Math.floor(s);
        else n.textContent = Math.floor(s);
        if (s >= tg) clearInterval(t);
      }, 16);
    }
...
function buildTestis(testis) {
  const track = document.getElementById('ttrack'); const dots = document.getElementById('tdots');
...
function goTesti(i) {
  const testis = gd('testis'); testiIdx = i;
...
function startTesti() {
  ...
  testiTimer = setInterval(() => goTesti((testiIdx+1) % gd('testis').length), 5000);
}
...
buildTestis(gd('testis')); startTesti();

// ── PROVINCIAS ──
function buildProvs(provs) {
  const list = document.getElementById('provList'); list.innerHTML = '';
  let visited = 0;
  provs.forEach(p => {
    if (p.v) visited++;
    const item = document.createElement('div'); item.className = 'pitem ' + (p.v?'v':'p');
    item.innerHTML = `<span class="pname">${p.name}</span><span class="ptag ${p.v?'v':'p'}">${p.v?'Visitada':'Pendiente'}</span>`;
    list.appendChild(item);
  });
  const pct = Math.round((visited / provs.length) * 100);
  document.getElementById('mapLegend').textContent = pct + '% de cobertura';
  document.getElementById('mbarFill').dataset.pct = pct;
  document.getElementById('mapPct').dataset.pct = pct;
  document.getElementById('mapNum').dataset.prov = visited;
}
buildProvs(gd('provs'));
...
function buildAgenda(events) {
  ...
  events.forEach((e, i) => {
    const d = document.createElement('div'); d.className = 'ev rv';
    ...
    grid.appendChild(d); io.observe(d);
  });
}
...
function applyVotoImg(src) {
  ...
  box.innerHTML = `<img src="${src}" alt="Cédula de votación" style="width:100%;border-radius:8px;display:block">`;
}
...
// ── FALLBACK: ensure all sections visible ──
setTimeout(() => {
