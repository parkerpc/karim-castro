...
function submitVol() {
  ...
  const phone='51'+fw.value.trim();
  const prov=fp.value;
  const roles=checks.length>0?checks.join(', '):'sin especificar';

  // Build WhatsApp message to campaign number
  const info=(_config&&_config.info)||{};
  const campPhone=(info.phone||'999999999').replace(/\D/g,'');
  const fullCamp=campPhone.startsWith('51')?campPhone:'51'+campPhone;

  const msg=encodeURIComponent(
    `¡Hola! Quiero ser voluntario/a de campaña de Karim Castro N°2 🗳️
...
`+
    `📍 Provincia: ${prov}
`+
    `✅ Puedo ayudar en: ${roles}`
  );

  document.getElementById('volWaBtn').href=`https://wa.me/${fullCamp}?text=${msg}`;
