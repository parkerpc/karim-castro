...
async function loadConfig() {
  try {
    ...
    const rawRes = await fetch(rawUrl, { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } });
    if (rawRes.ok) {
...
async function refreshConfigForAdmin() {
  try {
    const rawUrl = `https://raw.githubusercontent.com/${GH.owner}/${GH.repo}/main/data/config.json?t=${Date.now()}`;
    const res = await fetch(rawUrl, { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } });
    if (res.ok) {
